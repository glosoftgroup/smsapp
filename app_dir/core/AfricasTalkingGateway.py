...
class AfricasTalkingGateway:
    ...
    def deleteSubscription(self, phoneNumber_, shortCode_, keyword_):
        if len(phoneNumber_) == 0 or len(shortCode_) == 0 or len(keyword_) == 0:
            raise AfricasTalkingGatewayException("Please supply phone number, "
    ...
    def fetchPremiumSubscriptions(self, shortCode_, keyword_, lastReceivedId_ = 0):
        ...
        url = "%s?username=%s&shortCode=%s&keyword=%s&lastReceivedId=%s" % \
                 (self.getSmsSubscriptionUrl(), self.username,
                  shortCode_, keyword_, lastReceivedId_)
    ...
    # Payment Methods
    def initiateMobilePaymentCheckout(self, productName_, phoneNumber_, currencyCode_,
                                      amount_, metadata_):
        parameters = {
            'username': self.username,
